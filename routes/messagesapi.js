...
// Helper: Read messages (with file creation if missing)
const readMessages = async () => {
  try {
    await fs.access(MESSAGES_FILE); // check if file exists
  } catch {
    await fs.writeFile(MESSAGES_FILE, JSON.stringify([], null, 2)); // create empty file
    return [];
  }
